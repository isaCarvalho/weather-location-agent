    ...
    resolveId(id, importer) {
      ...
      try {
        ...
        for (const ext of [".mjs", ".js", ".cjs"]) {
          ...
          if (resolved2) {
            const pkgName = chunk3GJSIS6L_cjs.getPackageName(id);
            if (!pkgName) {
              return null;
            }
...
async function getInputOptions2(entryFile, platform, env, { sourcemap = false } = {}) {
  ...
  const { workspaceMap, workspaceRoot } = await chunkQ5FR6PUS_cjs.getWorkspaceInformation({ mastraEntryFile: entryFile });
  const analyzeEntryResult = await chunkQ5FR6PUS_cjs.analyzeBundle(
    [entryFile],
    entryFile,
    {
  ...
  for (const [dep, metadata] of analyzeEntryResult.dependencies.entries()) {
    const pkgName = chunk3GJSIS6L_cjs.getPackageName(dep);
  ...
  if (Array.isArray(inputOptions.plugins)) {
    ...
    inputOptions.plugins.push(chunkQ5FR6PUS_cjs.aliasHono());
...
function removeAllOptionsExceptServer(result, logger) {
  return chunkQ5FR6PUS_cjs.removeAllOptionsFromMastraExcept(result, "server", logger);
}

// src/build/serverOptions.ts
async function getServerOptions(entryFile, outputDir, logger) {
  const result = await chunkQ5FR6PUS_cjs.extractMastraOption(
    "server",
    entryFile,
...
//# sourceMappingURL=chunk-VQLBK6M5.cjs.map
//# sourceMappingURL=chunk-VQLBK6M5.cjs.map