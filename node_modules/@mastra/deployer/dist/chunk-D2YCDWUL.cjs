...
var chunkXJ42VWDB_cjs = require('./chunk-XJ42VWDB.cjs');
var chunk3EKA4UZF_cjs = require('./chunk-3EKA4UZF.cjs');
...
var Bundler = class extends bundler.MastraBundler {
  ...
  async writeInstrumentationFile(outputDirectory, customInstrumentationFile) {
    ...
    const __dirname = path.dirname(url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('chunk-D2YCDWUL.cjs', document.baseURI).href))));
  ...
  createBundler(inputOptions, outputOptions) {
    return chunk3EKA4UZF_cjs.createBundler(inputOptions, outputOptions);
  }
  async analyze(entry, mastraFile, outputDirectory, { enableEsmShim = true } = {}) {
    return await chunkQ5FR6PUS_cjs.analyzeBundle(
      [].concat(entry),
      mastraFile,
      {
  ...
  async getBundlerOptions(serverFile, mastraEntryFile, analyzedBundleInfo, toolsPaths, { enableSourcemap = false, enableEsmShim = true } = {}) {
    const { workspaceRoot } = await chunkQ5FR6PUS_cjs.getWorkspaceInformation({ mastraEntryFile });
    ...
    const inputOptions = await chunk3EKA4UZF_cjs.getInputOptions(
      mastraEntryFile,
      analyzedBundleInfo,
      "node",
      {
  ...
  async _bundle(serverFile, mastraEntryFile, {
    ...
    let sourcemap = false;
    try {
      const bundlerOptions = await chunkQ5FR6PUS_cjs.getBundlerOptions(mastraEntryFile, analyzeDir);
    ...
    try {
      ...
      analyzedBundleInfo = await chunkQ5FR6PUS_cjs.analyzeBundle(
    ...
    try {
      const result = await chunkXJ42VWDB_cjs.writeTelemetryConfig(mastraEntryFile, path.join(outputDirectory, this.outputDir));
...
//# sourceMappingURL=chunk-D2YCDWUL.cjs.map
//# sourceMappingURL=chunk-D2YCDWUL.cjs.map